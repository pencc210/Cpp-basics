...
class Organism{
    ...
    public:
    Organism(int row, int col): row(row), col(col){}
    //Setters
    ...
    void move(Organism& o, char (&arr)[gridRows][gridCols]){
        ...
        arr[o.getRow()][o.getCol()] = 'o'; //Marker for new org position 
        cout << "After moving: " << o.getRow() << ", " << o.getCol() << endl;

    }

    void breed(int row, int col, char (&arr)[gridRows][gridCols], int time, vector<Organism>& organisms) {
...
void initialSpawn(int antNum, int doodleNum, char arr[][gridCols], vector <Organism>& organisms);
...
int main(){
    ...
    initialSpawn(antNum, doodleNum, grid, organisms); //Place initial number of ants and dbugs on grid
    ...
    while (true){
        ...
        printGrid(grid, gridCols);
            ...
            for(auto& o : organisms){ //Move each org
                o.move(o, grid);  
            }

            for (auto& o : organisms) { // Breed each org
                o.breed(o.getRow(), o.getCol(), grid, i, organisms);
            }
...
void initialSpawn(int antNum, int doodleNum, char arr[][gridCols], vector <Organism>& organisms){
    ...
    for(int i = 0; i < antNum; i++){
        ...
        Organism o(row, col); //Create organism
        organisms.push_back(o); //Add to organism vector
    }
